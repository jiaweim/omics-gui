 ...
 */
public class PTMEditor extends SplitPane implements SettingView, ShowAlert
{
//    private final PTMFactory ptmFactory = PTMFactory.getInstance();
    ...
    private PTMSetting ptmSetting = null;
    ...
    public void setPtmSetting(PTMSetting ptmSetting)
    {
        this.ptmSetting = ptmSetting;
        updateView(ptmSetting);
    }
    ...
    private void initPTMTable()
    {
        ...
        massCol.setCellFactory(new Callback<>()
        {
            @Override
    ...
    private void initSpecificityEdit()
    {
        ...
        spAANode.setConverter(new StringConverter<>()
        {
            @Override
    ...
    private void initSpecificityTable()
    {
        ...
        nlMassCol.setCellFactory(new Callback<>()
        {
            @Override
    ...
    private void delPTM()
    {
        PTM selectedItem = ptmTableNode.getSelectionModel().getSelectedItem();
        if (selectedItem == null)
            return;

        ptmSetting.remove(selectedItem);
        ptmTableNode.getItems().remove(selectedItem);
        ptmTableNode.getSelectionModel().clearSelection();
    }
    ...
    private void addPTM()
    {
        ...
        if (ptmSetting.contains(ptmName)) {
            showAlert(Alert.AlertType.ERROR, "PTM of name '" + ptmName + "' already exists!");
            return;
        }
        ...
        if (StringUtils.isEmpty(formula)) {
            showAlert(Alert.AlertType.ERROR, "The composition is not defined!");
            return;
        }
        ...
        if (items.isEmpty()) {
            showAlert(Alert.AlertType.ERROR, "Specificity is not defined!");
            return;
        }
    ...
    private void savePTMToFile()
    {
        ...
        if (file != null) {
            ptmSetting.saveTo(file.getAbsolutePath());
        }
    }
    ...
    {
        this.ptmSetting = (PTMSetting) setting;
        ptmTableNode.getItems().setAll(ptmSetting.getPTMList());
    }
}
